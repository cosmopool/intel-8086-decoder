...
int main() {
  ...
  u_int8_t op_code = (buffer[0] & 0xFC) >> 2;
  switch (op_code) {
  case MOV:
    instruction = "mov";
    break;
  ...
  u_int8_t d_bit = (buffer[0] & (1 << 2));
  // printf("D: %d\n", d_bit != 0);
  (void)d_bit;

  // check if W bit is set
  u_int8_t w_bit = (buffer[0] & 1);
  // printf("W: %d\n", w_bit != 0);
  (void)w_bit;

  // MOD. extract bits 7 to 6 from second byte
  u_int8_t mod = (buffer[1] & 0xC0) >> 6;
  switch (mod) {
  case 3:
    break;
